...
void checkBestHydrationSize(int &helPercen,hydrationShellMinimal &hydrationShellin){
...
{  
  ...
  *************************************/
  ...
  helRatList.push_back(0.4);
  helRatList.push_back(0.6);
  helRatList.push_back(0.8);
  double helixRatio;
  int solventsPerLink =1;
  //double idealSolRat=std::atof(argv[15]);
  ...
  for(int i=0;i<mol.size();i++){
    for(int j=0;j<helRatList.size();j++){
      ...
      hydrationShellTmp.tubeParamList();
      hydrationShellTmp.constructInitialState();
      hydrationShellTmp.getAllHelices();
      std::vector<double> solDistsTmp;
      std::vector<double> solMolDistsTmp;
      hydrationShellTmp.solventMoleculeDistances(solMolDistsTmp,solDistsTmp);
      std::vector<std::vector<point> > solptsin = hydrationShellTmp.returnFlatSolList();
      int molSizeinTmp = mol[i].getNoAminos();
      int noSolin=0;
      for(int l=0;l<solptsin.size();l++){
	noSolin = noSolin + solptsin[l].size();
      }
      double closeToIdeal;
      double solRat = double(noSolin)/double(molSizeinTmp);
      double closeToIdealNew  = std::abs(solRat-idealSolRat);
      if(j==0){
	helixRatio=helRatList[0];
	closeToIdeal = closeToIdealNew;
      }else{
	if(closeToIdealNew<closeToIdeal){
	  closeToIdeal = closeToIdealNew;
	  helixRatio=helRatList[j];
	}
      }
    }
  ...
  std::cout<<"inital overlap penalty "<<overlapPenalty<<"\n";
	...
	for(int j=0;j<mol[l].getSubsecSize(i)-1;j++){
	  ...
	  if((doAll==true) || (std::find(fixedSecLists[l].begin(),fixedSecLists[l].end(),totalIndex)!=fixedSecLists[l].end())){
	    ...
	    if(cacaDist==false){
	      ...
	      // try to figure out the new best hydration layer parameters.
	      for(int jm=0;jm<helRatList.size();jm++){
		hydrationShellMinimal hydrationShellTmp(molCopy,Rin,Rout,RShell,ntrivs,helRatList[jm],solventsPerLink,closestApproachDist,rmin,rmax,lmin);
		hydrationShellTmp.tubeParamList();
		hydrationShellTmp.constructInitialState();
		hydrationShellTmp.getAllHelices();
		std::vector<double> solMolDistsCopyTmp;
		std::vector<double> solDistsCopyTmp;
		hydrationShellTmp.solventMoleculeDistances(solMolDistsCopyTmp,solDistsCopyTmp);
		std::vector<std::vector<point> > solptsin = hydrationShellTmp.returnFlatSolList();
		int molSizeinTmp = molCopy.getNoAminos();
		int noSolin=0;
		for(int l=0;l<solptsin.size();l++){
		  noSolin = noSolin + solptsin[l].size();
		}
		...
		double solRat = double(noSolin)/double(molSizeinTmp);
		double closeToIdealNew  = std::abs(solRat-idealSolRat);
		//std::cout<<"solRatTest "<<solRat<<" "<<helRatList[jm]<<"\n";
		if(jm==0){
		  helixRatio=helRatList[0];
		  closeToIdeal = closeToIdealNew;
		}else{
		  if(closeToIdealNew<closeToIdeal){
		    closeToIdeal = closeToIdealNew;
		    helixRatio=helRatList[jm];
		  }
		}
	      }
	      //std::cout<<"helix ratio cool ? "<<helixRatio<<"\n";
		...
		//std::cout<<"wr stat "<<wrPen<<" "<<newWrithe<<" "<<originalWrithes[l]<<"\n";
